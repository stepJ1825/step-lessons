...
---------------------------------
XML to Java config.
1. После сдачи предыдущего задания ветка develop должна содержать работающий код с XML конфигурацией.
2. От ветки 'develop' нужно ответвится и создать новую ветку 'feature/java-config'
3. Переписать XML конфигурацию на чистый Java код c использованием аннотаций @Configuration, \
@ComponentScan, @Bean и других необходимых.
4. Удалить файл application.xml и убедиться в корректности работы кода.
5. Создать пул-реквест 'feature/java-config'>'develop'.
6. Сбросить пул-реквест в группу в ТГ и получить аппрув или замечания.
7. После получения аппрува выполнить слияние.
---------------------------------
1. Написанное на Java-config приложение будет находиться в 'develop' ветке.
2. От ветки 'develop' нужно ответвится и создать новую ветку 'feature/web-controller'
3. Переписать слой контроллеров с использованием аннотаций @Controller и @RestController, (ОБА ВАРИАНТА) \
соответственно отдавать на запросы нужно JSON и готовый HTMl. \
В контроллерах для передачи данных использовать PathVariable, RequestParam и т.д.
4. Добавить POST и/или PUT-PATCH и/или DELETE запросы в контроллеры \
для изменения сущностей.
5. Создать пул-реквест 'feature/web-controller'>'develop'.
6. Сбросить пул-реквест в группу в ТГ и получить аппрув или замечания.
7. После получения аппрува выполнить слияние.
