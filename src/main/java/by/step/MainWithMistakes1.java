...
public class MainWithMistakes1 {
    ...
    private static void task3() throws IOException {
        ...
        animals.stream().filter(animal -> animal.getAge() > 30)
                ...
                .distinct()
                .map(origin -> origin.charAt(0))
                .peek(firstChar -> System.out.println(firstChar))
                .forEach(System.out::println);
    }
    ...
    private static void task8() throws IOException {
        ...
        int maxAge = animals.stream()
                ...
                .max()
                .orElse(0);
        System.out.println("Самое старое из сотни : " + maxAge + " лет");
    }
    ...
    private static void task11() throws IOException {
        System.out.println("\nЗадание 11");
        List<Animal> animals = Util.getAnimals();
        double averageAgeFromIndonesia = animals.stream()
    ...
    private static void task13() throws IOException {
        ...
        //TODO:
        LocalDate today = LocalDate.now();
        Function<House, Stream<? extends Person>> houseStreamFunction = house -> house.getPersonList()
                .stream();
        List<Person> allPeoples = houses.stream()   // первый прогон
                .flatMap(houseStreamFunction)
                .toList();
        List<Person> firstPriority = houses.stream()    // второй прогон
                .filter(house -> "Hospital"
                        .equalsIgnoreCase(house.getBuildingType()))
                .flatMap(houseStreamFunction).toList();
        List<Person> secondPriority = allPeoples.stream()   // третий прогон
                .filter(person -> {
                    int age = Period.between(person.getDateOfBirth(), today).getYears();
                ...
                .toList();
        List<Person> thirdPriority = allPeoples.stream() // 4ый прогон
                .filter(person -> !firstPriority.contains(person)
                        && !secondPriority.contains(person))
                .toList();
        List<Person> evacuationList = Stream.of(firstPriority,
                        secondPriority,
                        thirdPriority)
    ...
    private static void task14() throws IOException {
        ...
        Map<String, Integer> countryMassMap = new LinkedHashMap<>();

        List<Car> forSent = new ArrayList<>(cars);

        List<Car> toTurkmenistan = forSent.stream()
                .filter(car -> (car.getCarModel().equalsIgnoreCase("Jaguar")
                        || car.getColor().equalsIgnoreCase("White")))
                .toList();
        Integer toTurkmenistanMass = toTurkmenistan.stream()
                .mapToInt(Car::getMass)
                .sum();

//        countryMap.put("Туркменистан", toTurkmenistan);
        countryMassMap.put("Туркменистан", toTurkmenistanMass);
...
//        }
        totalExpenditure = countryMassMap.entrySet()
                .stream()
                .map(entry ->
                        new AbstractMap.SimpleEntry<>(entry.getKey(), entry.getValue() * 7.14 / 1000))
                .peek(entry -> System.out.printf(entry.getKey()
